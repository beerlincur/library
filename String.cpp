...
unsigned String::len() const//метод получения длины строки
{
	return length_;
}

int String::index(char c) const//метод получения индекса элемента строки
{
	for (size_t j = 0; j < length_; j++)
	{
		if (data_[j] == c)
...
void String::togglecase(unsigned first, unsigned last)//верхние к нижнему, нижние к верхнему size_t
{
	if ((first >= last) || (last > length_))
	{
		throw WRONG_INDEX;
...
istream& operator>> (istream& is, String& str)//считывание
{
...
char String::operator[] (unsigned j) const//получение элемента строки
{
	if (j > length_)
	{
		throw WRONG_INDEX;
...
String operator+ (const String& left, const String& right)//сложения
{
